...
//            toAddString = strDate + "    " + toAddString;
    ...
    private void saveItemsToDatabase() {
        ...
        new saveItemsToDatabase(this, toDoItemDao).execute();
    }
    ...
    private static class readItemsFromDatabase extends AsyncTask<Void, Void, List<ToDoItem>>{
        ...
        public readItemsFromDatabase(MainActivity activity, ToDoItemDao dao){
        ...
        @Override
        protected List<ToDoItem> doInBackground(Void... voids) { // ... == vargars == Variable Arguments
            //read items from database
            List<ToDoItem> itemsFromDB = toDoItemDao.listAll();
            return itemsFromDB;
        }

        protected void onPostExecute(List<ToDoItem> itemsFromDB ) {

            if (itemsFromDB != null & itemsFromDB.size() > 0) {
                for (ToDoItem item : itemsFromDB) {
                    mainActivity.items.add(item.getToDoItemName());
    ...
    private static class saveItemsToDatabase extends AsyncTask<Void, Void, Void>{
        ...
        public saveItemsToDatabase(MainActivity activity, ToDoItemDao dao){
            mActivityRef = new WeakReference<>(activity);
            toDoItemDao = dao;
